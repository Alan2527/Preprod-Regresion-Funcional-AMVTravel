...
from pages.front_servicios_page import FrontServiciosPage as P
...
def test_reserva_servicio_flujo_completo(logged_in_driver):
    ...
    with allure.step("1 a 5. Seleccionar pestaña Servicios, ingresar destino, tipo y buscar"):
        try:
            ...
            tab_servicios = wait.until(EC.element_to_be_clickable(P.TAB_SERVICIOS), message="No se encontró la pestaña a[href='#tabServices']")
            tab_servicios.click()
            time.sleep(1)

            # 3. Dropdown Destino (Ciudad)
            btn_ciudad = wait.until(EC.element_to_be_clickable(P.DDL_CIUDAD), message="No se encontró el control del DDL de Ciudad")
            btn_ciudad.click()
            time.sleep(1)  # Esperamos que se despliegue la lista
            opcion_bariloche = wait.until(EC.element_to_be_clickable(P.OPCION_BARILOCHE), message="No se encontró la opción 'Bariloche' en la lista")
            opcion_bariloche.click()
            time.sleep(1)

            # 4. Dropdown Tipo de Servicio (Opcionales)
            btn_tipo = wait.until(EC.element_to_be_clickable(P.DDL_TIPO), message="No se encontró el control del DDL de Tipo de Servicio")
            btn_tipo.click()
            time.sleep(1)  # Esperamos que se despliegue la lista
            opcion_excursion = wait.until(EC.element_to_be_clickable(P.OPCION_EXCURSION), message="No se encontró la opción 'Excursión' en la lista")
            ...
            # 5. Click en Buscar
            wait.until(EC.element_to_be_clickable(P.BTN_SEARCH), message="No se encontró el botón de Buscar (btnSearch)").click()

            # Espera larga para los resultados
            wait_largo = WebDriverWait(driver, 45)
            wait_largo.until(EC.presence_of_element_located(P.CARD), message="La búsqueda superó los 45 segundos y no cargaron los resultados de servicios")
            time.sleep(2)
    ...
    with allure.step("6. Validar panel de resultados (Cards de Servicios)"):
        try:
            ...
            assert driver.find_elements(*P.CARD), "No se encontró ningún div con la clase 'panelShadow col-sm-6'"

            # b, c, d, e, f. Validar components internos de la card
            assert driver.find_elements(*P.CARD_IMG), "Falta la imagen de 450x323px en la card"
            assert driver.find_elements(*P.CARD_NOMBRE), "Falta el nombre del servicio (h4Span)"
            assert driver.find_elements(*P.CARD_TABLA), "Falta la tabla de precios/tipo centrada"
            assert driver.find_elements(*P.CARD_DESC), "Falta la descripción del servicio (divservlimit)"
            assert driver.find_elements(*P.CARD_BTN), "Falta el botón de selección/ver más"
    ...
    with allure.step("7 y 8. Click en servicio y validar detalle interno"):
        try:
            # 7. Click en el botón
            btn_seleccionar = wait.until(EC.element_to_be_clickable(P.CARD_BTN), message="No se encontró el botón para entrar al servicio")
            btn_seleccionar.click()
            time.sleep(3)  # Espera a que cargue el detalle

            # 8. Validaciones de la vista de detalle
            assert wait.until(EC.presence_of_element_located(P.DET_NOMBRE), message="No cargó el Detalle del Servicio (falta h3ServiceName)")
            assert driver.find_elements(*P.DET_IMG), "Falta la imagen del servicio (popu)"
            assert driver.find_elements(*P.DET_DETALLES), "Faltan los detalles del servicio (detailsdiv)"
            assert driver.find_elements(*P.DET_TABLA), "Falta la tabla de precios detallada"
    ...
    with allure.step("9 y 10. Seleccionar pasajeros"):
        try:
            ...
            select_pax_1 = Select(wait.until(EC.presence_of_element_located(P.DD_PAX_1), message="No se encontró el primer DDL de pasajeros"))
            select_pax_1.select_by_visible_text("2")

            # 10. Segundo select (Pax 2)
            select_pax_2 = Select(wait.until(EC.presence_of_element_located(P.DD_PAX_2), message="No se encontró el segundo DDL de pasajeros"))
    ...
    with allure.step("11 y 12. Confirmar reserva y validar incremento en el carrito"):
        try:
            ...
            cart_element = wait.until(EC.presence_of_element_located(P.CART_COUNT), message="No se encontró el contador del carrito")
            ...
            for _ in range(5):
                try:
                    btn_reservar = wait.until(EC.element_to_be_clickable(P.BTN_RESERVAR))
                    break
                except Exception:
            ...
            wait.until(
                lambda d: int(d.find_element(*P.CART_COUNT).text.strip() or 0) == expected_count,
    ...
    with allure.step("13. Ir al carrito y Finalizar"):
        try:
            cart_anchor = wait.until(EC.visibility_of_element_located(P.CART_ANCHOR))
            actions.move_to_element(cart_anchor).perform()
            time.sleep(1)

            btn_finalizar = wait.until(EC.element_to_be_clickable(P.BTN_FINALIZAR))
    ...
    with allure.step("14. Completar Referencia y Comentarios"):
        try:
            wait.until(EC.presence_of_element_located(P.DET_TABLA))

            driver.find_element(*P.INPUT_REFERENCIA).send_keys(texto_referencia)

            fecha_hoy = datetime.now().strftime("%d/%m/%Y")
            driver.find_element(*P.INPUT_COMENTARIO).send_keys(f"Este es un test autómatico ejecutado el día {fecha_hoy}")

            allure.attach(driver.get_screenshot_as_png(), name="Referencia_Completada", attachment_type=allure.attachment_type.PNG)

            btn_success = wait.until(EC.element_to_be_clickable(P.BTN_SUCCESS))
            btn_success.click()
        except Exception as e:
    ...
    with allure.step("15. Cargar Comentarios de Servicio y Datos de Pasajeros"):
        try:
            ...
            input_comentario1 = wait.until(EC.presence_of_element_located(P.INPUT_COMENTARIO_1))
            input_comentario1.send_keys("Comentario 1")

            driver.find_element(*P.INPUT_COMENTARIO_2).send_keys("Comentario 2")

            driver.find_element(*P.PAX_NAME).send_keys("Alan")
            driver.find_element(*P.PAX_SURNAME).send_keys("QA")
            driver.find_element(*P.PAX_PASSPORT).send_keys("PAS123456789")
            driver.find_element(*P.PAX_BIRTHDAY).send_keys("02061990")
            driver.find_element(*P.PAX_NATIONALITY).send_keys("Argentino")

            pax_q = driver.find_element(*P.PAX_QUANTITY)
            pax_q.clear()
            pax_q.send_keys("2")

            checkbox = driver.find_element(*P.CHK_TERMINOS)
            ...
            btn_guardar = wait.until(EC.presence_of_element_located(P.BTN_CONFIRMAR))
    ...
    with allure.step("16. Validación final de éxito en pestaña Booking"):
        try:
            ...
            tab_booking = wait_largo.until(
                EC.element_to_be_clickable(P.TAB_BOOKING),
            ...
            wait_largo.until(
                EC.presence_of_element_located(P.TABLA_BOOKING),
