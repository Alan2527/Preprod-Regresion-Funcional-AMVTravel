...
from pages.multidestino_page import MultidestinoPage
from pages.multidestino_page import MultidestinoPage as P
...
def test_reserva_multidestino(logged_in_driver):
    ...
    try:
        ...
        with allure.step("1 y 2. Navegar a la sección de Multidestinos"):
            btn_multidestino = wait.until(EC.element_to_be_clickable(P.LINK_MULTIDESTINO))
            driver.execute_script("arguments[0].click();", btn_multidestino)
            mp.esperar_fin_de_carga()
        ...
        with allure.step("3 y 4. Buscar 'Triángulo Argentino (8 días / 7 noches)'"):
            input_buscador = wait.until(EC.visibility_of_element_located(P.INPUT_BUSCADOR))
            input_buscador.clear()
            input_buscador.send_keys("Triángulo Argentino (8 días / 7 noches)")
            time.sleep(1)

            btn_buscar = wait.until(EC.element_to_be_clickable(P.BTN_BUSCAR))
        ...
        with allure.step("5. Validar estructura del resultado de búsqueda"):
            card = wait.until(EC.presence_of_element_located(P.CARD))
            img_view = wait.until(EC.presence_of_element_located(P.CARD_IMG))
            details_left = wait.until(EC.presence_of_element_located(P.CARD_DETALLES))
        ...
        with allure.step("6. Clic en el botón para cotizar"):
            btn_cotizar = wait.until(EC.element_to_be_clickable(P.BTN_COTIZAR))
        ...
        with allure.step("7. Configurar Fecha (Hoy + 7 días) y Pasajeros"):
            ...
            input_fecha = wait.until(EC.presence_of_element_located(P.INPUT_FECHA))
            ...
            Select(wait.until(EC.presence_of_element_located(P.DD_PAX))).select_by_value("6")
            time.sleep(1)
            Select(wait.until(EC.presence_of_element_located(P.DD_SGL))).select_by_value("1")
            Select(wait.until(EC.presence_of_element_located(P.DD_DBL))).select_by_value("1")
            Select(wait.until(EC.presence_of_element_located(P.DD_TPL))).select_by_value("1")
        ...
        with allure.step("8. Clic en botón rosa para calcular/continuar"):
            btn_calcular = wait.until(EC.element_to_be_clickable(P.BTN_CALCULAR))
        ...
        with allure.step("9. Quitar dinámicamente todos los servicios 'No disponible'"):
            ...
            while intentos < intentos_maximos:
                servicios_no_disp = driver.find_elements(*P.SERVICIOS_NO_DISP)
        ...
        with allure.step("10. Agregar servicio opcional (Delta Premium)"):
            btn_add = wait.until(EC.element_to_be_clickable(P.BTN_ADD_SERVICIO))
            driver.execute_script("arguments[0].scrollIntoView({block:'center'});", btn_add)
            driver.execute_script("arguments[0].click();", btn_add)
            mp.esperar_fin_de_carga()

            tabla_servicios = wait.until(EC.presence_of_element_located(P.TABLA_SERVICIOS_AGREGADO))
        ...
        with allure.step("11. Cambiar idioma a Inglés en la 3er fila y validar recargo"):
            h6_precio = wait.until(EC.presence_of_element_located(P.TERCER_PRECIO))
            ...
            dropdown_elemento = wait.until(EC.presence_of_element_located(P.TERCER_IDIOMA))

            select_idioma = Select(dropdown_elemento)
            select_idioma.select_by_visible_text("Inglés")
            mp.esperar_fin_de_carga()

            h6_precio_actualizado = wait.until(EC.presence_of_element_located(P.TERCER_PRECIO))
        ...
        with allure.step("12 y 13. Clic en Reservar y esperar pantalla"):
            btn_reservar = wait.until(EC.presence_of_element_located(P.BTN_RESERVAR))
            ...
            wait.until(EC.presence_of_element_located(P.BTN_SAVE_BOOK))
        ...
        with allure.step("14. Llenar datos de vuelo y observaciones"):
            ...
            input_vuelo = wait.until(EC.visibility_of_element_located(P.INPUT_VUELO))
            driver.execute_script("arguments[0].scrollIntoView({block:'center'});", input_vuelo)
            input_vuelo.send_keys(f"TEST{dia_hoy}")

            input_hora = driver.find_element(*P.INPUT_HORA)
            input_hora.send_keys("10:30")

            input_obs = driver.find_element(*P.INPUT_OBS)
        ...
        with allure.step("15. Llenar datos de pasajero y aceptar términos"):
            input_nombre = wait.until(EC.visibility_of_element_located(P.INPUT_NOMBRE))
            driver.execute_script("arguments[0].scrollIntoView({block:'center'});", input_nombre)
            input_nombre.send_keys("Alan")

            input_apellido = driver.find_element(*P.INPUT_APELLIDO)
            input_apellido.send_keys("Test Automático")

            chk_terminos = wait.until(EC.presence_of_element_located(P.CHK_TERMINOS))
        ...
        with allure.step("16 y 17. Guardar reserva y validar creación exitosa en tabla"):
            btn_save = wait.until(EC.element_to_be_clickable(P.BTN_SAVE_BOOK))
