...
from pages.bo_payorder_page import BoPayOrderPage as P
...
def test_crear_orden_pago(login_bo):
    driver = login_bo
    ...
    with allure.step("2.1 Ir a sección PayOrders"):
        ...
        safe_click(wait, P.LINK_PAYORDERS)
    ...
    with allure.step("3. Buscar y Seleccionar Proveedor MAX BAIRES"):
        safe_click(wait, P.BTN_SUPPLIER)
        search = wait.until(EC.visibility_of_element_located(P.INPUT_SEARCH))
        search.clear()
        search.send_keys("MAX BAIRES")

        fila_proveedor = wait.until(EC.element_to_be_clickable(P.FILA_PROVEEDOR))
    ...
    with allure.step("8. Ingresar Monto"):
        safe_send_keys(wait, P.TXT_AMOUNT, "900000")
    ...
    with allure.step("9. Guardar Registro"):
        ...
        errores = driver.find_elements(*P.VALIDATION_ERRORS)
    ...
    with allure.step("11. Click en Asignar Total"):
        ...
        try:
            btn_asignar_total = wait.until(EC.presence_of_element_located(P.LNK_ASIGNAR_TOTAL))
        except Exception:
            ...
            candidatos = driver.find_elements(*P.CANDIDATOS_ASIGNAR)
    ...
    with allure.step("12 y 13. Validar existencia de celda en tabla interna"):
        ...
        celda_tabla = wait.until(EC.presence_of_element_located(P.CELDA_TABLA_INTERNA))
    ...
    with allure.step("15. Click en Aprobar Orden de Pago"):
        boton_aprobar = wait.until(EC.presence_of_element_located(P.BTN_APPROVE))
        ...
        wait.until(EC.invisibility_of_element_located(P.BTN_APPROVE))
